...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const [index, row] of rows.entries()) {
      ...
      requireFields(
        row,
        ["election_code", "constituency_name", "candidate_name", "party_code"],
        rowNumber,
        issues
      );
      ...
      if (!party) issues.push({ rowNumber, field: "party_code", message: "Party not found" });
      ...
      if (!dry_run) {
        const { data: candidate, error: candidateError } = await admin
          ...
          .upsert(
            {
              normalized_name: normalizeCandidateName(row.candidate_name),
        ...
        const { error: electionCandidateError } = await admin
          ...
          .upsert(
            {
              ...
              source_status: row.source_status || "imported",
              notes: row.notes || null
            },
            { onConflict: "election_id,constituency_id,candidate_id,party_id" }
          );

        if (electionCandidateError) throw electionCandidateError;
        imported += 1;
      }
    }
